	...
	<div class="containor lgConbox">
		...
		<div class="conBox">
			<form class="form-horizontal" name="formByPhone" id="formByPhone">
				<input ng-model="radioType" type="radio" name="authNum" value="byPhone" ng-checked="radioType == byPhone" ng-change="resetForm(formByPhone)">가입시 등록한 휴대전화로 인증
			...
			<form class="form-horizontal" id="formByEmail" name="formByEmail">
				<input ng-model="radioType" type="radio" name="authNum" value="byEmail" ng-change="resetForm(formByEmail)">가입시 등록한 이메일로 인증
				...
				<div class="joinBox" ng-show="radioType == 'byEmail'">
					...
					<div class="form-group">
						<label for="userReginum" class="col-xs-2 col-sm-2 control-label">인증코드</label>
						<div class="col-xs-10 col-sm-10">
							<input type="text" class="form-control leftInput col-xs-12" id="userReginum" placeholder="인증코드 5자리를 입력해주세요" ng-model="userReginum" name="userReginum" placeholder="" ng-pattern="/^\d+$/" ng-maxlength="5">
							<button type="submit" class="btn btn-trace rightBtn col-xs-12" ng-disabled="isSent == false || expired == true" ng-click="confirmEcode()" ng-hide="ecodeValid == true">인증번호확인</button>
							<button type="submit" class="btn btn-trace rightBtn col-xs-12" ng-disabled="ecodeValid == true" ng-show="ecodeValid == true">인증완료</button>
						</div>
					</div>
					<div class="alertBox">
			...
			<div class="btnBox">
				<button class="btn btn-lg btn-primary" type="submit" ng-disabled="smsValid == false && ecodeValid == false" ng-click="searchUser()">확인</button>
				<button ui-sref="login" class="btn btn-lg btn-trace" type="submit">취소</button>
				</div>
			</div>
