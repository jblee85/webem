'use strict';
angular.module('iceApp').controller('pvDetail_ctrl',[ '$scope','$state','$rootScope', '$window','apiService','DTOptionsBuilder', 'DTColumnDefBuilder', '$sce', 'DEFINED','MODEL','$interval',
...
//			apiService.put_ESSChargingStatus("r18081601", "l18081601", cmdModel)
			.then(
					function(response){
						if(response.result){
							$scope.type = type;
							...
							console.log($scope.pcsData.st);
//							console.log($scope.mOutput);
						    if(reqSt == $scope.pcsData.st){
//						    	console.log("inppkw: " + $scope.pcsData.inppkw);
						    	if(value == $scope.pcsData.inppkw){ //input 값 비교
						    		...
						    		$window.setTimeout(function(){
							    		if(value != $scope.pcsData.inppkw){
							    			console.log("출력값: "+value + " / pcs값: "+ $scope.pcsData.inppkw);
...
//		console.log(typeof(mOutput));
//		console.log($scope.maxpkw);
...
//		setInterval(function () {
		...
		.then(
			function(response) {
				$scope.timeList = [];
				for(var i = 0; i<response.length; i++){
//					var cDate = response[i].cdt.substring(0,10);
